//! The node format. `synapse-write-node.sh` produced a node and
//! `synapse-query.sh` read one, with every rule below duplicated in bash
//! and a comment asking the copies to stay in step -- a shared
//! implementation is what retires that comment.
//!
...
//! ```
//!
//! The fence is the contract with the human: everything between the markers
//! is regenerated on every write; everything after `synapse:generated:end`
...
///
/// A configured boilerplate chain (`src/main/java`, etc.) carries no
/// subsystem info, so a path containing one is cut at the segment before
/// it. A flat `<pkg>/src/<subsystem>/…` layout has no such chain, and
/// there the segment *after* `src/` is the subsystem. Failing both, the
/// first path component; failing that, `(repo root)`.
///
/// `chains` comes from `~/.claude/synapse-module-boilerplate.conf`; an
/// empty list still groups, just without collapsing scaffolding.
pub fn moduleOf(path: []const u8, chains: []const []const u8) []const u8 {
    for (chains) |chain| { // first match wins, in conf order
        if (chain.len == 0) continue;
        // Searched as `/chain/`, so `src/main` won't fire on `mysrc/maintenance`.
    ...
    if (std.mem.indexOf(u8, path, "/src/")) |at| {
        ...
        if (std.mem.indexOfScalar(u8, rest, '/')) |slash| {
            return path[0 .. at + 5 + slash]; // base/src/<subsystem>, contiguous in the input
        }
        return base;
    }
...
pub const Source = struct {
    path: []const u8,
    /// 40 hex characters, as written in the node. Kept as text, not parsed.
    hash: []const u8,
};

/// `sha256` over `LC_ALL=C`-sorted `path:hash` lines, joined by newlines
/// with no trailing one. Returns 64 lowercase hex characters, the form the
/// node stores.
...
pub const Split = struct {
    ...
    head: []const u8,
    /// From the end marker onward (`## Notes` and after). Empty when the
    /// node has no end marker -- written before the fence existed.
    tail: []const u8,
    /// True when both markers were found. A node missing them is not an
    /// error: the writer replaces the whole body, as it did pre-fence.
    fenced: bool,
};

/// Where the generated region begins and ends, so a rewrite can replace it
/// and keep everything else byte-for-byte.
...
test "sources_digest is sha256 over sorted path:hash lines" {
    ...
    // Out of order on input -- the digest sorts, so order must not matter.
    ...
    const c = try sourcesDigest(gpa, &.{ // a changed hash changes the digest
...
test "an empty source list digests to sha256 of nothing" {
    ...
    try testing.expectEqualStrings( // sha256(""), a well-defined digest, not an empty field
