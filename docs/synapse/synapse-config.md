...
| `SYNAPSE_VAULT_STORE` | `disk` | Which `Store` backend `resolveStore()` resolves to: `disk` (plain markdown files only, reads `SYNAPSE_VAULT_DIR` directly as a folder — no Obsidian dependency at all, including its own `LinkGraph`/`Renamer`) or `obsidian` (plain disk I/O for `read`/`write`/`list`; `search` and the `LinkGraph`/`Renamer` capabilities go through the official `obsidian` CLI when it's reachable, falling back to `disk`'s own otherwise). Any other value is a hard error, not a silent fallback. |
