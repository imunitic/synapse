...
| `SYNAPSE_VAULT_STORE` | `obsidian` | Which `Store` backend `resolveStore()` resolves to: `obsidian` (plain disk I/O for `read`/`write`/`list`; `search` and the `LinkGraph`/`Renamer` capabilities go through the official `obsidian` CLI when it's reachable, falling back to `disk`'s own otherwise) or `disk` (plain markdown files only, reads `SYNAPSE_VAULT_DIR` directly as a folder — no Obsidian dependency at all, including its own `LinkGraph`/`Renamer`). Any other value is a hard error, not a silent fallback. |
...
| `SYNAPSE_GRAMMARS_QUERY_PATH` | `Extractor.query_override_dir` | Directory holding hand-authored `{ext}.scm` files that preempt the entire tier cascade for that extension, tags.scm included — checked fresh every run, never cached, for the rare grammar neither automatic tier handles well. Same directory also holds `{ext}.locals.scm`, the analogous override for local-reference filtering (see synapse-code-cache.md), checked before the grammar's own real `locals.scm`. |
