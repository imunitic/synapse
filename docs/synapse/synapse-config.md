...
| `SYNAPSE_VAULT_STORE` | `obsidian` | Which `Store` backend `resolveStore()` resolves to: `obsidian` (plain disk I/O for `read`/`write`/`list`, the official `obsidian` CLI for `search`/the link graph) or `disk` (plain markdown files only, reads `SYNAPSE_VAULT_DIR` directly as a folder, no `LinkGraph`). Any other value is a hard error, not a silent fallback. |
