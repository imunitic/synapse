...
`synapse-db-sync.sh` auto-commits every vault edit, so the intact version is one
...
`synapse/{repo}@{branch}/` is not free-form notes. Nodes are written by `synapse-write-node.sh`
...
them. `_index.json`, `_refs.tsv` and `_tags_cache.bin` are machine-only. `_profile.txt` and
`_manifest.tsv` are human-readable and may be edited directly.
