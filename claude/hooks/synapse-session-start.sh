...
# `synapse-hook session-start` -- see src/apps/hook/session_start.zig. Identity stays here
...
# The payload carries `cwd`, and identity is resolved from it -- so the wrapper has
# to read stdin here and replay it to the binary. `jq` for one field only; a hook
# with no jq exits 0 like any other missing precondition.
...
printf '%s' "$INPUT" | exec "$HOOK_BIN" session-start
